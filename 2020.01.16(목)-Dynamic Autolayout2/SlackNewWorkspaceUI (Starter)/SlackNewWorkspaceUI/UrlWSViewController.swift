...
final class UrlWSViewController: UIViewController {
  ...
  private let placeholderLabel: UILabel = {
    let label = UILabel()
    label.text = ".slack.com" //플레이스 홀더 느낌나게 레이블을 세팅해주었다.
  ...
  private let errorMessageLabel: UILabel = {
    ...
    label.isHidden = true //평소에는 히든 상태
    return label
  }()
  ...
  private func setupConstraints() {
    ...
    let textSize = (workspaceName as NSString).size(withAttributes: [.font: urlTextField.font!]) //workspaceName에 따라서 글자의 너비를 구하는거
    placeholderLeadingConst = placeholderLabel.leadingAnchor
      .constraint(equalTo: urlTextField.leadingAnchor, constant: textSize.width)//constant: textSize.width는 글자의 너비만큼 이동하겠다는거
  ...
  @objc private func didTapNextButton(_ sender: UIButton) { //nextButton 셀렉티드일때만 활성화 시키고
    guard nextButton.isSelected, !["error", "fail"].contains(urlTextField.text!) else {
      shakeAnimation()
      errorMessageLabel.isHidden = false
      AudioServicesPlaySystemSound(kSystemSoundID_Vibrate) //진동기능,  위에 임포트 추가해라
      return
    }
